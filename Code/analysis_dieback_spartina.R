...
dieback_2009_vector$Condition <- "Unvegetated"
...
f_dieback_nocreek_no_NA_2009$Dieback[f_dieback_nocreek_no_NA_2009$value  > 0.75] <- "Unvegetated"
...
#####Repeat for all years to make Fig 1
...
jpeg("../Paper/Figures/FigS3.jpeg", res = 300, width = 3000, height = 2000)
...
dieback_2021_df$dieback_2021_factor[dieback_2021_df$dieback_2021 >= 0.75] <- "Unvegetated"
...
dieback_2019_df$dieback_2019_factor[dieback_2019_df$dieback_2019 >= 0.75] <- "Unvegetated"
...
dieback_2017_df$dieback_2017_factor[dieback_2017_df$dieback_2017 >= 0.75] <- "Unvegetated"
...
dieback_raster_2016 <- raster(paste0(wd, "../Multispectral/Aerial_Imagery/NAIP/Dieback_probability/Dieback_prob_2016_nocreek.tif"))

DEM_LIDAR_corrected_reproject <- projectRaster(DEM_raster_corrected, dieback_raster_2016) 
tidal_distance_raster_reproject <- projectRaster(tidal_distance_raster, dieback_raster_2016) 

DEM_values <- values(DEM_LIDAR_corrected_reproject)
distance_creek_values <- values(tidal_distance_raster_reproject)
dieback_2016_values <- values(dieback_raster_2016)

dieback_2016_df  <- tibble(DEM = DEM_values,
                           distance_creek = distance_creek_values,
                           dieback_2016 = dieback_2016_values,
                           Year = 2016)

dieback_2016_df <- dieback_2016_df[complete.cases(dieback_2016_df),]
dieback_2016_df <- dieback_2016_df[dieback_2016_df$DEM >= -0.06,]
dieback_2016_df <- dieback_2016_df[dieback_2016_df$DEM <= 1.1,]
dieback_2016_df <- dieback_2016_df[dieback_2016_df$dieback_2016 > -0.5,]

dieback_2016_df$dieback_2016_factor <- ""
dieback_2016_df$dieback_2016_factor[dieback_2016_df$dieback_2016 >= 0.75] <- "Unvegetated"
dieback_2016_df$dieback_2016_factor[dieback_2016_df$dieback_2016 <= 0.25] <- "Vegetated"

dieback_2016_df$dieback_2016_factor <- as.factor(dieback_2016_df$dieback_2016_factor)

dieback_2016_df_midmarsh <- dieback_2016_df[dieback_2016_df$distance_creek > 7.5,]
dieback_2016_df_creekbank <- dieback_2016_df[dieback_2016_df$distance_creek <= 7.5,]
...
dieback_2015_df$dieback_2015_factor[dieback_2015_df$dieback_2015 >= 0.75] <- "Unvegetated"
...
dieback_2013_df$dieback_2013_factor[dieback_2013_df$dieback_2013 >= 0.75] <- "Unvegetated"
...
dieback_2011_df$dieback_2011_factor[dieback_2011_df$dieback_2011 >= 0.75] <- "Unvegetated"
...
dieback_2009_df$dieback_2009_factor[dieback_2009_df$dieback_2009 >= 0.75] <- "Unvegetated"
...
names(dieback_2009_df) = names(dieback_2011_df) = names(dieback_2013_df) =
  names(dieback_2015_df) = names(dieback_2016_df) = names(dieback_2017_df) = names(dieback_2019_df) =
...
dieback_df <- rbind(dieback_2009_df, dieback_2011_df, dieback_2013_df,
                    dieback_2015_df, dieback_2016_df, dieback_2017_df, 
                    dieback_2019_df, dieback_2021_df)
...
field_data_curated$dieback_factor[field_data_curated$biomass_total == 0] <- "Unvegetated"
...
summary(field_data_curated[field_data_curated$dieback_factor == "Unvegetated",]$delta_DEM)
...
Dieback_percentage_plot <- ggplot(dieback_df_summary_DEM_group_raw[dieback_df_summary_DEM_group_raw$dieback_factor == "Unvegetated",],
...
Drought_index_df <- Drought_index_df[Drought_index_df$Year_continuous > 2008.49,]
...
precipitation_df  <- read.csv("../Field data/precipitation_Charleston.csv")
precipitation_df$precipitation_mm <- precipitation_df$precipitation_inches*25.4

precipitation_df <- precipitation_df[precipitation_df$Year > 2008.49 & precipitation_df$Year < 2021.51, ]

prec_fig <- ggplot(precipitation_df, aes(x = Year, y = precipitation_mm)) + geom_point() + 
  geom_line() + theme_bw() +
  scale_x_continuous(breaks = c(2009, 2011, 2013, 2015, 2017, 2019, 2021)) + 
  theme(axis.text.x = element_text(size = 8)) +
  labs(x = "Year", y = "Monthly precipitation (mm)")

ggarrange(Dieback_percentage_plot, PDSI_fig, prec_fig, ncol = 1, nrow = 3, align = "hv")


##Partial Fig 1
tiff("../Paper/Figures/Fig_1_R_new.tiff", res = 300 , width = 1250, height = 2100)
ggarrange(Dieback_percentage_plot, PDSI_fig, prec_fig, ncol = 1, nrow = 3, align = "hv")
dev.off()

##Dieback increase fig 2

#See dieback_df_summary_DEM_group_raw.txt file

fig2_db <- dieback_df_summary_DEM_group_raw[dieback_df_summary_DEM_group_raw$dieback_factor == "Dieback",]
fig2_db_creek <- fig2_db[fig2_db$marsh_location == "Creekbank",]
fig2_db_midmarsh <- fig2_db[fig2_db$marsh_location == "Midmarsh",]

fig2_db_total <- aggregate(fig2_db$Percetage, by=list(Year=fig2_db$Year), FUN=sum)
names(fig2_db_total)[length(fig2_db_total)] <- "Percetage"

fig2_db_creek$Percetage_change <- NA
for(i in 2:length(fig2_db_creek$Percetage)) {
  fig2_db_creek$Percetage_change[i] <- 100*(fig2_db_creek$Percetage[i] - fig2_db_creek$Percetage[i-1])/fig2_db_creek$Percetage[i-1]
}

fig2_db_midmarsh$Percetage_change <- NA
for(i in 2:length(fig2_db_midmarsh$Percetage)) {
  fig2_db_midmarsh$Percetage_change[i] <- 100*(fig2_db_midmarsh$Percetage[i] - fig2_db_midmarsh$Percetage[i-1])/fig2_db_midmarsh$Percetage[i-1]
}

fig2_db_total$Percetage_change <- NA
for(i in 2:length(fig2_db_total$Percetage)) {
  fig2_db_total$Percetage_change[i] <- 100*(fig2_db_total$Percetage[i] - fig2_db_total$Percetage[i-1])/fig2_db_total$Percetage[i-1]
}

fig2_db_total <- fig2_db_total %>%
  tibble::add_column(marsh_location = "Total marsh area",
             .after = "Year") 

fig2_db_fig <- rbind(fig2_db_creek[,c(1,3,10,11)], fig2_db_midmarsh[,c(1,3,10,11)], fig2_db_total)

fig2_db_fig <- fig2_db_fig[fig2_db_fig$Year != 2009,]

fig2_db_fig$marsh_location <- as.factor(fig2_db_fig$marsh_location)
fig2_db_fig$marsh_location <- factor(fig2_db_fig$marsh_location,
                              levels(fig2_db_fig$marsh_location)[c(3,2,1)])


fig_2 <- ggplot(fig2_db_fig, aes(x = Year, y = Percetage_change)) + geom_bar(position="stack", stat="identity") +
  facet_wrap(~marsh_location, scales = "fixed", nrow = 3, ncol = 1) +
  theme_bw() + labs(y = "Increase in unvegetated area (%)") + 
...
##Fig 2
tiff("../Paper/Figures/Fig_2_R_new.tiff", res = 600 , width = 2000, height = 3000, compression = "lzw")
fig_2
dev.off()

##Dieback characterization - Fig3

df_quantile_distance <- aggregate(dieback_df_curated$distance_creek, 
                                   list(dieback_df_curated$dieback_factor,
                                        dieback_df_curated$Year,
                                        dieback_df_curated$marsh_location),
                                   FUN='quantile') 

df_quantile_distance <- do.call(data.frame, df_quantile_distance) 

names(df_quantile_distance) <- c("dieback_factor", "Year", "marsh_location",
                                  "quantile_0", "quantile_25", "quantile_50",
                                  "quantile_75", "quantile_100")

df_quantile_distance$Year <- as.numeric(as.character(df_quantile_distance$Year))

fig_distance <- ggplot(df_quantile_distance, aes(x = Year, y = quantile_50, color = dieback_factor)) +
  geom_point(alpha = 0.8) + theme_bw() + geom_line(alpha = 0.8) + 
  geom_errorbar(aes(ymin = quantile_25, ymax = quantile_75), 
                width = 0.3, alpha = 0.8) +
...
df_quantile_elevation <- aggregate(dieback_df_curated$DEM, 
                                   list(dieback_df_curated$dieback_factor,
                                        dieback_df_curated$Year,
                                        dieback_df_curated$marsh_location),
                                   FUN='quantile') 

df_quantile_elevation <- do.call(data.frame, df_quantile_elevation) 

names(df_quantile_elevation) <- c("dieback_factor", "Year", "marsh_location",
                                  "quantile_0", "quantile_25", "quantile_50",
                                  "quantile_75", "quantile_100")

df_quantile_elevation$Year <- as.numeric(as.character(df_quantile_elevation$Year))


fig_elevation <- ggplot(df_quantile_elevation, aes(x = Year, y = quantile_50, color = dieback_factor)) +
  geom_point(alpha = 0.8) + theme_bw() + geom_line(alpha = 0.8) + 
  geom_errorbar(aes(ymin = quantile_25, ymax = quantile_75), 
                width = 0.3, alpha = 0.8) +
  facet_wrap(~marsh_location, scales = "free") + 
  expand_limits(y = 0) + labs(y = "2017 Elevation NAVD88 (m)") +
  theme(legend.title = element_blank(), axis.title = element_text(size = 16),
...
jpeg("../Paper/Figures/Fig3.jpeg", res = 300, width = 3000, height = 2000)
...
Rmisc::summarySE(data_september, measurevar = "Biomass_total", groupvars = "Year")
...
field_data_curated$distance_factor <- "Midmarsh"
field_data_curated$distance_factor[field_data_curated$tidal_distance < 7.5] <- "Creek bank"

tiff("../Paper/Figures/Biomass_vs_creek.tiff", res = 300, width = 1750, height = 1250, compression = "lzw")
ggplot(field_data_curated, aes(x = tidal_distance, y = biomass_total, color = distance_factor)) + 
  geom_point() + theme_bw() + geom_vline(aes(xintercept = 7.5), linetype = "dashed") +
  labs(x = "Distance from tidal creek (m)",
       y = expression(paste("Aboveground biomass (", g~m^-2, ")"))) +
